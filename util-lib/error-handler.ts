    ...
    return (error: Error, _request: Request, response: Response, _next: NextFunction) => {
      ...
      switch (error.name) {
        case 'ResourceExistError':
          response
            .status(HttpStatusCodesEnum.CONFLICT)
            .json(ResponseData.createValidationFailure(error.message))
            .end();
          break;
        case 'ResourceNotFoundError':
          response
            .status(HttpStatusCodesEnum.NOT_FOUND)
            .json(ResponseData.createResource404(error.message))
            .end();
          break;
        ...
        case 'ApiNotFoundError':
          response
            .status(HttpStatusCodesEnum.NOT_FOUND)
            .json(ResponseData.createApiResource404(error.message))
            .end();
          break;
        default:
          response
            ...
            .json(
              ResponseData.createInternalError(
                'Unable to complete operation due to an internal error.'
              )
            )
            .end();
      }
